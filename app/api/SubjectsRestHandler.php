...
class SubjectsRestHandler extends SimpleRest {
	...
	function getSubjectTeacher($id) {	
		...
		$teacher = new Subjects();
		$rawData = $teacher->getSubjectTeacher($id);
        
		if(empty($rawData)) {
			$statusCode = 404;
			$rawData = array('error' => 'No teacher found!');		
		} else {
			$statusCode = 200;
		}
	...
	function getTeacherClassSubjects($teacherId,$classId) {	
		...
		$subjects = new Subjects();
		$rawData = $subjects->getTeacherClassSubjects($teacherId,$classId);
